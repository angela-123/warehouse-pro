...
<html>
    ...
    <body>
        ...
        $recno = $_POST['invoice'];
        $cname = $_POST['cname'];
  //$tata = "select productname,sum(qtyout-returns) as qty,unitprice, discount,sum((qtyout-returns)*unitprice)-sum((qtyout-returns) * unitprice * 0.01 * discount) as amount from sales where recno = '$recno' and productname!='' group by productname";
  $tata = "select productname,sum(qtyout-returns) as qty,discount,unitprice,sum(qtyout-returns)*unitprice-sum((qtyout-returns) * unitprice * 0.01 * discount) as amount from sales where recno = '$recno'  group by productname";
 $sisi = "select sdate, productname,sum((qtyout-returns) * unitprice * 0.01 * discount) as discount from sales where recno = '$recno' group by recno";
 
 $tadau = "select customername, productname,sum(qtyout-returns)*unitprice-sum(qtyout-returns) * unitprice * 0.01 * discount as amount from sales where recno = '$recno' group by recno ";
 
 $data = "select productname,sum((qtyout-returns) * unitprice) as extended from sales where recno = '$recno' group by recno";
 $all = "select customername,staffname, productname,sum(qtyout) as qty,sum((qtyout-returns) * unitprice) as extended,sum(paid) as payment,sum(deposit) as deposits,sum(others) as others  from sales where recno = '$recno' group by recno";
