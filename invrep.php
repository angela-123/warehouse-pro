...
<html>
 ...
 $tash = "select * from sales where recno = '$recno' group by productname";
 $sisi = "select productname,sum((qtyout-returns) * unitprice * 0.01 * discount) as discount from sales where recno = '$recno' group by recno";
 ...
 $all = "select customername,staffname, productname,sum(qtyout-returns) as qty,sum((qtyout-returns) * unitprice) as extended,sum(paid) as payment,sum(deposit) as deposits,sum(others) as others  from sales where recno = '$recno' group by recno";
 
 $foo = mysql_query($tash);
 
 $rtp = mysql_fetch_assoc($foo);
