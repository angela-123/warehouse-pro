...
<html>
 ...
 $tata = "select productname,sum(qty-returns) as qty,rate,sum((qty-returns)*rate) as amount from audit where invno = '$recno'  group by productname";
 
 $sisi = "select adate, productname,sum((qty-returns) * rate * 0.01 * discount) as discount from audit where invno = '$recno' group by invno";
 
 $tadau = "select customername, productname,sum(qty-returns)*rate-sum((qty-returns) * rate * 0.01 * discount) as amount from audit where invno = '$recno' group by invno ";
 
 $data = "select productname,sum((qty-returns) * rate) as extended from audit where invno = '$recno' group by invno";
 $all = "select customername,doneby, productname,sum(qty-returns) as qty,sum((qty-returns) * rate) as extended,sum(payment) as payment  from audit where invno = '$recno' group by invno";
